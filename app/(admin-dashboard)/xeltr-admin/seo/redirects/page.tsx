...
export default function AdminRedirectsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const url = editId ? `/api/admin/seo/redirects/${editId}` : "/api/admin/seo/redirects";
    ...
    <>
      <AnimatePresence>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Form */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-[#020617] border border-white/10 rounded-3xl p-8 h-fit">
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                filteredRedirects.map((entry) => (
                  <motion.div key={entry._id} layout className={`group bg-[#020617] border rounded-2xl p-5 transition-all ${entry.enabled ? "border-white/10 hover:border-blue-500/30" : "border-red-500/10 opacity-60"}`}>
